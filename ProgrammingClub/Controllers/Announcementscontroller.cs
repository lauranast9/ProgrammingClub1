    ...
    public class AnnouncementsController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddAnnouncement([FromBody] Announcement announcement)
        {
            try
            {
                ...
                return StatusCode((int)HttpStatusCode.Created, SuccessMessages.AnnouncementAdded);
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal server error: {ex.Message}");
        ...
        public async Task<IActionResult> Put(Guid id, [FromBody] Announcement announcement)
        {
            try
            {
                ...
                {
                    return StatusCode((int)HttpStatusCode.OK, SuccessMessages.AnnouncementUpdated);
                }
        ...
        public async Task<IActionResult> PatchAnnouncement(Guid id, [FromBody] UpdateAnnouncementPartially announcement)
        {
            try
            {
                ...
                {
                    return StatusCode((int)HttpStatusCode.OK, SuccessMessages.AnnouncementAdded);
                }
                return StatusCode((int)HttpStatusCode.NotFound, ErrorMessagesEnum.AnnouncementNotFound);
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                ...
                return StatusCode((int)HttpStatusCode.OK, SuccessMessages.AnnouncementRemoved);
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal server error: {ex.Message}");
